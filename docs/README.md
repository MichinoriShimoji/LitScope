...
DOI が登録されているカードには「参考文献を取り込む」ボタンが出ます。押すと：

1. Crossref から親論文のメタデータを再取得して `reference` 配列を取り出す
2. 参考文献を1件ずつ自動検証
...
3. どちらも失敗したものは **未検証フラグ付き**で本体に追加（カードが黄色く強調表示）
4. 進行状況はステータスに「23/47 検証中…」のように表示
