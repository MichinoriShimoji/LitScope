...
1. ページを開く
2. 検索条件を入力 → **検索（孫引き拡張）** ボタンをクリック
3. 自動的に走る処理：
   - **Crossref + OpenAlex + CiNii Research** の3ソースを並列検索（日本語の書籍章・国内誌も拾える）
   - ヒットした各論文の **参考文献を自動抽出**（PDF優先 → Crossref フォールバック）
   - 取れた参考文献を1件ずつ再検証（DOI lookup / title 検索）
   - 確実な一致が取れたら「確認済」、ダメなら「未検証」として本体に追加
4. 黄色いカード（未検証）を編集して「確認済みにする」 or 「削除」
5. **ZIPでまとめてダウンロード** で完成版を取得
6. （任意）**Zoteroへ直接送信** で Zotero Desktop に取り込み

検索1回で「論文＋その参考文献」を一気にまとめて取り込みます。ステージ操作は不要。
